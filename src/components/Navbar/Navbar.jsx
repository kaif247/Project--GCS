  ...
  return (
    <header className={styles.navbar}>
      ...
      <nav className={styles.center}>
        {navItems.map(item => (
          <Link key={item.to} to={item.to} className={`${styles.navItem} ${location.pathname === item.to ? styles.active : ''}`} aria-label={item.label}>
            {item.icon}
          </Link>
        ))}
      </nav>
      <div className={styles.right}>
        <Link to="/create" className={styles.iconBtn} aria-label="Create">
          <FaPlusCircle />
        </Link>
        <Link to="/messenger" className={styles.iconBtn} aria-label="Messenger">
          <FaFacebookMessenger />
        </Link>
        <Link to="/notifications" className={styles.iconBtn} aria-label="Notifications">
          <FaBell />
        </Link>
        <Link to="/profile" className={styles.profileBtn} aria-label="Profile">
